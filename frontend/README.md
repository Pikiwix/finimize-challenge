...
Now you can rename `src/calculator.css` to `src/App.scss` and run `npm run watch-css`. The watcher will find every Sass file in `src` subdirectories, and create a corresponding CSS file next to it, in our case overwriting `src/calculator.css`. Since `src/App.js` still imports `src/calculator.css`, the styles become a part of your application. You can now edit `src/App.scss`, and `src/calculator.css` will be regenerated.
